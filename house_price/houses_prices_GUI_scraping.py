...
class OtoDomWebScraping(QObject):
    ...
    def base_info(self):
        link_base = r"https://www.otodom.pl/pl/oferty/sprzedaz/{rodzaj}/{city}?priceMin={min_price}&priceMax={max_price}&areaMin={min_area}&areaMax={max_area}&distanceRadius={radius}&limit=72&page=".format(
            ...
            city=self.city,
            radius=self.radius,
            min_price=self.new[0],
            max_price=self.new[1],
            min_area=self.new[2],
            max_area=self.new[3],
        )
    ...
    def WebScraping(self):
        ...
        for page in range(1, (pages + 1)):
            try:
                articles = soup.find_all("ul", {"class": "css-14cy79a e3x1uf06"})[
                    1
                ].select("article")
            except:
                time.sleep(5)
            ...
            hrefs = [
                "https://www.otodom.pl" + href["href"]
                for href in soup.find_all("a", {"class": "css-1s8gywf ek5ep0h1"})
            ]
            print(hrefs)
            # lists to store data
            ...
            for article_no, article_href in enumerate(hrefs):
                ...
                try:
                    ...
                    offer_title = (
                        articles[article_no]
                        .find_all("h3", {"class": "css-1oq8pdj es62z2j24"})[0]
                        .get_text()
                    )
                    # Price
                    item_price = (
                        off_page.find_all("strong", {"class": "css-b114we eu6swcv14"})[
                            0
                        ]
                        .get_text()
                        .replace("\xa0", "")[:-2]
                    ...
                    # Add data to list
                    data_to_add.extend(
                        [offer_title, item_price, localisation, article_href]
                    )
                    # parameters
                    parameters = off_page.find_all(
            ...
            for element in range(len(data_csv)):
                df = df.append(
                    pd.DataFrame([data_csv[element]], columns=column_names[element]),
                    sort=False,
                )
    ...
    def Convert_and_Clean_Date(self, filepath):
        ...
        df.drop(
            ["Dostępne od", "Czynsz", "Obsługa zdalna", "Materiał budynku", "Okna"],
            axis=1,
            inplace=True,
        )
        ...
        )  # Jesli nie ma 'None','None'

        locations_coordination = [
            list(element) for element in locations_coordination
        ]  # Convert to mutable (list)

        # removing  None
        indices = [
            index
            for index, value in enumerate(locations_coordination[0])
            if value == "None"
        ]
        ...
        manhattan_distance = [
            (abs(location[0] - city_center[0]) + abs(location[1] - city_center[1]))
            for location in zip(locations_coordination[0], locations_coordination[1])
        ]  # Obl. Manhattan Distance
        manhattan_distance = [
            element * 111 for element in manhattan_distance
        ]  # 1 degree = 111 kilometers

        manhattan = pd.Series(
            manhattan_distance, name="manhattan[km]"
        )  # Utworzenie serii z manhattan distance
        ...
        df["Powierzchnia"] = df["Powierzchnia"].apply(
        ...
        df.drop(
            ["Ogrzewanie", "Stan wykończenia", "Forma własności", "Rodzaj zabudowy"],
            axis=1,
            inplace=True,
        )
        # dropping duplicate columns
        ...
        df_clear = df_clear.rename(
            columns={
                "Forma własności_spółdzielcze wł. z KW": "Forma własności_spółdzielcze wł z KW"
            }
        )

        # removing min. and km
        df_clear["API_Google_Distance"] = df_clear["API_Google_Distance"].apply(
            lambda data: float(data.split(" ")[0])
        )
        df_clear["API_Google_Distance"] = df_clear["API_Google_Distance"].apply(
            lambda data: 0 if (data == 1) else data
        )
        ...
        def rm_sigma(data_frame, col_name="Cena", sigma=2):
            ...
            dataFrame = data_frame[
                (data_frame[col_name] < sigma_thresh_up)
                & (data_frame[col_name] > sigma_thres_down)
            ]
            return dataFrame
