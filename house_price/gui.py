...
class MyTableWidget(QWidget):
    def __init__(self):
        ...
        self.heading.addItems(
            ["gazowe", "elektryczne", "kotłowe", "miejskie", "piec kaflowy", "inne"]
        )
        self.type_building = QComboBox(self)
        self.type_building.addItems(
            [
                "apartamentowiec",
                "blok",
                "dom wolnostojący",
        ...
        self.distance.addItems(
            ["0 km", "+ 5 km", "+ 10 km", "+ 15 km", "+ 25 km", "+ 50 km", "+ 70 km"]
        )
        self.type = QComboBox(self)
        self.type.addItems(["mieszkanie", "dom", "dzialka"])
        self.form_of_the_property = QComboBox(self)
        self.form_of_the_property.addItems(
            [
                "pełna własność",
        ...
        self.timer.timeout.connect(
            self.save_button_disable_or_enable
        )  # zmienia status Save Disable_or_Enable
        self.timer.timeout.connect(
            self.start_scraping_button_disable_or_enable
        )  # zmienia status Start Scraping Disable_or_Enable
    ...
    def start_scraping_func(self):
        ...
        city_name = (
            self.city.text().replace("Ł", "L")
            if "Ł" in self.city.text()
            else (
                self.city.text().replace("ł", "l")
                if "ł" in self.city.text()
                else self.city.text()
        ...
        city = (
            unicodedata.normalize("NFKD", city_name)
            .encode("ascii", "ignore")
            .decode("utf8")
        )
        ...
        if (
            (price_min.isdigit() == True or price_min == "")
            and (price_max.isdigit() == True or price_max == "")
            and (area_min.isdigit() == True or area_min == "")
            and (area_max.isdigit() == True or area_max == "")
        ):
            pass
        else:
        ...
        self.obiekt = OtoDomWebScraping(
            city, radius, rodzaj, price_min, price_max, area_min, area_max
        )
    ...
    def messagebox_start(self):
        ...
        if msg_box.standardButton(msg_box.clickedButton()) == QMessageBox.Yes:
            ...
            self.worker = WorkerThread(
                self.parometers, self.path, self.name_Mongo, self.city
            )
    ...
    def messagebox_price(self):
        ...
        msg_box.setStyle(clean_looks)
        msg_box.setText(
            f"Price according to enter data is {str(int(self.price[0]))} PLN "
        )
    ...
    def load_mongo(self):
        ...
        self.data = [
            i for i in mongo_db.list_database_names() if "_db" in i or "rad" in i
        ]
        self.data_predict = [
            i.split("_")[0]
            for i in mongo_db.list_database_names()
            if "_db" in i or "rad" in i
        ]
    ...
    def tool_ptedict(self):
        ...
        if (
            (
                distans_to_centrum_pred.isdigit() == True
                and distans_to_centrum_pred != ""
            )
            and (rooms_pred.isdigit() == True and rooms_pred != "")
            and (level_pred.isdigit() == True and level_pred != "")
            and (level_in_block_pred.isdigit() == True and level_in_block_pred != "")
            and (enter_area_pred.isdigit() == True and enter_area_pred != "")
            and (year_of_build_pred.isdigit() == True and year_of_build_pred != "")
        ):
            pass
        else:
            return self.MessageBox_wrong_digital()

        price_result = Tool_Predict(
            city_pred,
...
class TableView(QTableWidget):
    def __init__(self, results):
        ...
        table_widget.setStyleSheet(
            "background-color: rgb(25, 25, 25); color: rgb(157, 168, 168)"
        )
